  ...
  return {
    info: {
      ...
      extractor: function(body, options, callback) {
        ...
        // Add project install.
        if (match = $('.project-info').text().match(/Reported installs\: ([0-9,]*)/m)) {
          data['install'] = parseInt(match[1].replace(/\,/g,''));
        }

        // Add project install.
        if (match = $('.project-info').text().match(/Downloads\: ([0-9,]*)/m)) {
          data['downloads'] = parseInt(match[1].replace(/\,/g,''));
        }
